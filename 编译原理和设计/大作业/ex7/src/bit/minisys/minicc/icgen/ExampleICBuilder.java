...
//		sb.print();
	...
	public void visit(ASTInitList initList) throws Exception {
		...
		Symbol sb = new Symbol();
		visit(initList.declarator);
		if(initList.declarator instanceof ASTArrayDeclarator){
			ASTArrayDeclarator arrayDeclarator = (ASTArrayDeclarator) initList.declarator;
			ASTVariableDeclarator variableDeclarator = (ASTVariableDeclarator) arrayDeclarator.declarator;
			Quat quat = new Quat("arr[]",null,variableDeclarator.identifier,(ASTNode) arrayDeclarator.expr);
			quats.add(quat);
		}
...
//		System.out.println(stringConst.value);
	...
	public void visit(ASTUnaryExpression unaryExpression) throws Exception {
		...
		Quat quat = new Quat(op,opnd2,opnd1,opnd2);
		quats.add(quat);
		map.put(unaryExpression,opnd2);
